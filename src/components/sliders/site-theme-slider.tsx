...
import useSetSiteTheme from "../../hooks/personal-info/set-site-theme"
import { usePersonalInfoContext } from "../../contexts/personal-info-context"

function SiteThemeSlider() {
	const personalInfoClass = usePersonalInfoContext()
	const setSiteTheme = useSetSiteTheme()
		...
		<div className="flex flex-col items-center space-y-2 z-20 mr-36">
			<label className="toggle-pill">
				<input
					...
					checked={personalInfoClass.siteTheme === "light"}
				...
				<span className="knob absolute left-1 bottom-1 h-6 w-6 bg-white rounded-full transition-transform duration-300"
					style={{ transform: personalInfoClass.siteTheme === "light" ? "translateX(13px)" : "translateX(0)" }}>
				</span>
			</label>
			<span className="text-sm font-medium text-white">
				{personalInfoClass.siteTheme.toUpperCase()}
			</span>
		</div>
	)
}
