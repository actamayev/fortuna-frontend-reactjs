...
function ReviewAskButton() {
	...
	const isAbleToAskForShares = useMemo(() => {
		if (_.isNull(exchangeClass) || _.isNull(positionsAndTransactionClass)) return false
		...
		const numberSharesHeld = positionsAndTransactionClass.getNumberSharesOwnedByUUID(videoUUID)
